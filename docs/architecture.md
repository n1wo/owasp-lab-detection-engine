...
server-side fetch, debug-endpoint, registration, admin-action, and profile-import
scenarios, and a detection engine that parses JSONL logs for brute-force,
SQLi-like, XSS-like, broken-access-control, SSRF, security-misconfiguration,
cryptographic-failure, logging-failure, and unsafe-object-trust pattern
detection.
...
profile import page, and structured logs for login outcomes, suspicious input,
admin access, outbound requests, configuration exposure, credential storage,
sensitive actions, and profile imports.
...
| `event_type` | `login_success`, `login_failure`, `account_lockout`, `suspicious_input`, `admin_access`, `outbound_request`, `config_exposure`, `credential_storage`, `sensitive_action`, `profile_import`, or `lab_mode_change` |
...
`CRYPTO-WEAK-001`, `LOG-GAP-001`, and `INTEGRITY-DESERIALIZE-001`. The engine
should focus on defensive detection behavior rather than offensive instructions.

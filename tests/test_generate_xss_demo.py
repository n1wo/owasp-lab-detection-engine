"""Tests for the localhost-only XSS-like comment demo activity generator."""
...
import generate_xss_demo as demo  # noqa: E402
...
class FakeOpener:
    def open(self, request, timeout=3.0):
        """Return a fake successful comment response without network access."""
...
def test_validate_local_base_url_allows_localhost_and_loopback():
    """Verify the XSS demo accepts only intended local hostnames."""
...
def test_validate_local_base_url_rejects_non_local_targets():
    """Verify the XSS demo refuses non-local or malformed base URLs."""
...
def test_send_comment_attempt_records_success_status(monkeypatch):
    """Verify insecure-mode success responses are returned as statuses."""

    monkeypatch.setattr(demo, "build_opener", lambda: FakeOpener())

    status = demo.send_comment_attempt("http://127.0.0.1:8080")

    assert status == 200


def test_send_comment_attempt_records_expected_http_error_status(monkeypatch):
...
def test_main_prints_helpful_message_for_unexpected_status(monkeypatch, capsys):
    """Verify the CLI explains stale app builds when /comment is unavailable."""

    monkeypatch.setattr(demo, "app_is_available", lambda base_url, timeout=3.0: True)
    monkeypatch.setattr(demo, "send_comment_attempt", lambda base_url, timeout=3.0: 404)
